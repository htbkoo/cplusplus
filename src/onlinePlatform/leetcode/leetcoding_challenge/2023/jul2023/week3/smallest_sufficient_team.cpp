...
public:
    vector<int> smallestSufficientTeam(vector<string>& req_skills, vector<vector<string>>& people) {        
        memo = vector<vector<long long>>();
        for (int i = 0; i < people.size(); ++i) {
            memo.push_back(vector<long long>(getBitmaskSize(req_skills), UNINITIALIZED));
        }
...
private:
    ...
    long long findSmallest(int i, int bitmask, vector<string>& req_skills, vector<vector<string>>& people) {
        ...
        if (memo[i][bitmask] == UNINITIALIZED) {
            long long skipPerson = findSmallest(i + 1, bitmask, req_skills, people);
            long long includePerson = withPerson(i, findSmallest(i + 1, withSkills(people[i], bitmask), req_skills, people));

            if (getNumPeople(skipPerson) <= getNumPeople(includePerson)) {
                memo[i][bitmask] = skipPerson;
            } else {
                memo[i][bitmask] = includePerson;
            }
        }
    ...
    int withSkills(vector<string>& skills, int bitmask) {
